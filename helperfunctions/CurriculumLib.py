...
#read entry works. Create the 3D volume for input to the nn
    def readEntry_new(self, idx):
        ...
        if 'Dikablis' in archName or 'OVM6211' in archName:
            more_features = True
...
def readArchives(path2arc_keys):
    ...
    for chunk in D:
        ...
        AllDS['subject_id'].append(subject_id.reshape(-1))

    # Concat all entries into one giant list
    for key, val in AllDS.items():
        AllDS[key] = np.concatenate(val, axis=0)
    # Convert scipy-loaded 0-d string arrays to plain strings
    for key in ('archive', 'subject_id'):
        AllDS[key] = np.array([str(x.flat[0]) if hasattr(x, 'flat') and not isinstance(x, str) else str(x)
                                for x in AllDS[key].flat])
    return AllDS
