...
def fix_batch(data_dict):
    is_bad = np.array(data_dict['is_bad'])
    loc_bad = np.where(is_bad)[0]
    loc_good = np.where(~is_bad)[0]

    # If the entire batch is bad (e.g. short sample sessions with no valid
    # 4-frame window), there is nothing to replace with -> signal the caller
    # to skip/resample the batch instead of crashing on np.random.choice([]).
    if len(loc_good) == 0:
