...
def forward(net,
    ...
    for bt in range(batches_per_ep):
        ...
        if torch.any(data_dict['is_bad']):
            ...
            data_dict, all_bad = fix_batch(data_dict)
            if all_bad:
                # Whole batch is unusable (no valid samples to draw from).
                # Skip this iteration rather than crash on an empty choice.
                # NOTE: safe for single-GPU; under DDP this would desync ranks.
                continue
